import { describe, expect, it } from "vitest";
import {
  SpeakerBaseline,
  computeRms,
  detectPitchHz,
  interpretVocalState,
  rmsToDb,
  summarizeFrames,
  type VocalFrame
} from "./audioAnalysis";

const SAMPLE_RATE = 44100;

/** A pure sine wave at a known frequency, for pitch-detection checks. */
function sine(freqHz: number, amplitude = 0.5, length = 2048): Float32Array {
  const out = new Float32Array(length);
  for (let i = 0; i < length; i += 1) {
    out[i] = amplitude * Math.sin((2 * Math.PI * freqHz * i) / SAMPLE_RATE);
  }
  return out;
}

function frames(spec: Array<Partial<VocalFrame>>): VocalFrame[] {
  return spec.map((frame) => ({ rms: frame.rms ?? 0, pitchHz: frame.pitchHz ?? null }));
}

function baselineFrom(rms: number, pitch: number): SpeakerBaseline {
  const baseline = new SpeakerBaseline();
  for (let i = 0; i < 40; i += 1) baseline.observe({ rms, pitchHz: pitch });
  return baseline;
}

describe("computeRms", () => {
  it("is zero for silence", () => {
    expect(computeRms(new Float32Array(1024))).toBe(0);
  });

  it("orders louder above quieter", () => {
    expect(computeRms(sine(150, 0.8))).toBeGreaterThan(computeRms(sine(150, 0.2)));
  });

  it("matches the known RMS of a sine (amplitude / sqrt 2)", () => {
    const rms = computeRms(sine(150, 1));
    expect(Math.abs(rms - 1 / Math.SQRT2)).toBeLessThan(0.02);
  });
});

describe("rmsToDb", () => {
  it("is finite for pure silence", () => {
    expect(Number.isFinite(rmsToDb(0))).toBe(true);
  });

  it("rises with loudness", () => {
    expect(rmsToDb(0.5)).toBeGreaterThan(rmsToDb(0.05));
  });
});

describe("detectPitchHz", () => {
  it("recovers a low male-range pitch", () => {
    const pitch = detectPitchHz(sine(120), SAMPLE_RATE);
    expect(pitch).not.toBeNull();
    expect(Math.abs((pitch ?? 0) - 120)).toBeLessThan(6);
  });

  it("recovers a higher pitch", () => {
    const pitch = detectPitchHz(sine(240), SAMPLE_RATE);
    expect(pitch).not.toBeNull();
    expect(Math.abs((pitch ?? 0) - 240)).toBeLessThan(10);
  });

  it("returns null for silence", () => {
    expect(detectPitchHz(new Float32Array(2048), SAMPLE_RATE)).toBeNull();
  });

  it("rejects pitch outside the speech band", () => {
    // 2 kHz is well above the voiced range and must not be reported as F0.
    expect(detectPitchHz(sine(2000), SAMPLE_RATE)).toBeNull();
  });
});

describe("summarizeFrames", () => {
  it("reports full silence", () => {
    const metrics = summarizeFrames(frames([{ rms: 0 }, { rms: 0 }]));
    expect(metrics.silentFraction).toBe(1);
    expect(metrics.meanPitchHz).toBeNull();
  });

  it("computes energy and pitch stats over voiced frames", () => {
    const metrics = summarizeFrames(
      frames([
        { rms: 0.2, pitchHz: 150 },
        { rms: 0.4, pitchHz: 170 },
        { rms: 0 }
      ])
    );
    expect(metrics.frameCount).toBe(3);
    expect(Math.abs(metrics.silentFraction - 1 / 3)).toBeLessThan(1e-9);
    expect(Math.abs(metrics.meanEnergy - 0.3)).toBeLessThan(1e-9);
    expect(metrics.peakEnergy).toBe(0.4);
    expect(metrics.meanPitchHz).toBe(160);
    expect(metrics.pitchVariabilityHz ?? 0).toBeGreaterThan(0);
  });
});

describe("interpretVocalState", () => {
  it("flags a mostly-silent window as a long pause and nothing else", () => {
    const metrics = summarizeFrames(frames(Array(10).fill({ rms: 0 })));
    const out = interpretVocalState(metrics, baselineFrom(0.2, 150), "A");
    expect(out).toHaveLength(1);
    expect(out[0].label).toBe("long-pause");
  });

  it("stays quiet until the speaker baseline is established", () => {
    const metrics = summarizeFrames(frames([{ rms: 0.9, pitchHz: 300 }]));
    const out = interpretVocalState(metrics, new SpeakerBaseline(), "A");
    expect(out).toHaveLength(0);
  });

  it("detects a raised voice against a quiet baseline", () => {
    const metrics = summarizeFrames(frames(Array(10).fill({ rms: 0.5, pitchHz: 160 })));
    const out = interpretVocalState(metrics, baselineFrom(0.2, 155), "B");
    expect(out.some((o) => o.label === "raised-voice")).toBe(true);
    expect(out[0].subject).toBe("B");
  });

  it("detects a tense voice from a pitch lift with modest extra energy", () => {
    const metrics = summarizeFrames(frames(Array(10).fill({ rms: 0.26, pitchHz: 210 })));
    const out = interpretVocalState(metrics, baselineFrom(0.2, 150), "A");
    expect(out.some((o) => o.label === "tense-voice")).toBe(true);
  });

  it("detects flat / withdrawn delivery: quiet, monotone, gappy", () => {
    const spec = [
      ...Array(5).fill({ rms: 0.1, pitchHz: 150 }),
      ...Array(5).fill({ rms: 0 })
    ];
    const out = interpretVocalState(summarizeFrames(frames(spec)), baselineFrom(0.25, 150), "A");
    expect(out.some((o) => o.label === "flat-withdrawn")).toBe(true);
  });

  it("detects warm / engaged delivery: lively pitch at easy volume", () => {
    const spec = [
      { rms: 0.2, pitchHz: 140 },
      { rms: 0.22, pitchHz: 190 },
      { rms: 0.2, pitchHz: 160 },
      { rms: 0.23, pitchHz: 210 },
      { rms: 0.2, pitchHz: 150 }
    ];
    const out = interpretVocalState(summarizeFrames(frames(spec)), baselineFrom(0.2, 170), "A");
    expect(out.some((o) => o.label === "warm-engaged")).toBe(true);
  });

  it("an ordinary, level sentence produces no observation", () => {
    const metrics = summarizeFrames(frames(Array(10).fill({ rms: 0.2, pitchHz: 155 })));
    const out = interpretVocalState(metrics, baselineFrom(0.2, 150), "A");
    expect(out).toHaveLength(0);
  });
});
