/**
 * Live vocal (prosody) analysis from the microphone, alongside the transcript.
 *
 * The words tell you *what* was said; the voice tells you *how*. Two partners can
 * say the same thing in ways that mean opposite things. This module reads the
 * "how" directly from the audio signal — no transcript, no cloud, no heavy model.
 *
 ...
 *                            baseline is a raised voice.
 *   - Pitch (F0)           → fundamental frequency via autocorrelation; a lift
 *                            over baseline tracks tension and escalation.
 *   - Pitch variability    → a flat, monotone line reads as withdrawal; lively
 *                            variation reads as engagement.
 *   - Silence fraction     → long gaps mark pauses, hesitation, or shutdown.
 *
 ...
 *
 * The pure functions are unit-tested in src/audioAnalysis.test.ts. The
 * `VocalAnalyser` class at the bottom is the thin Web Audio wrapper (untested;
 * it needs a real AudioContext — the logic it leans on is covered).
 *
 * This complements acousticFeatures.ts: that one summarises a *finished*
 * recording (coverage, pauses, level shifts); this one streams *live* vocal
 * states, with pitch, mapped to interaction families the couple can see.
 */

import type { PartnerId, VocalStateLabel } from "./types";
...
/** A vocal observation before it is stamped with an id and a timestamp. */
 ...
 * moment, or an ordinary sentence produces nothing. Evidence is written in
 * Hebrew for the UI.
 */
export function interpretVocalState(
  ...
  if (metrics.silentFraction > 0.85) {
    out.push({
      ...
      evidence: `שקט לאורך ${Math.round(metrics.silentFraction * 100)}% מהקטע`,
      metadata: { silentFraction: round(metrics.silentFraction, 2) }
    });
    return out;
  }
  ...
  if (meanEnergyRatio > 1.6 && energyRatio > 1.9) {
    out.push({
      ...
      evidence: `הקול חזק בכ-${Math.round(meanEnergyRatio * 100 - 100)}% מהרגיל`,
      metadata: { loudnessRatio: round(meanEnergyRatio, 2) }
    });
  }
  ...
  if (basePitch && metrics.meanPitchHz && metrics.meanPitchHz > basePitch * 1.15 && meanEnergyRatio > 1.2) {
    out.push({
      ...
      evidence: `גובה הקול עלה לכ-${Math.round(metrics.meanPitchHz)} הרץ, מעל ${Math.round(basePitch)} הרץ הרגילים`,
      metadata: { pitchHz: Math.round(metrics.meanPitchHz), basePitchHz: Math.round(basePitch) }
    });
  }
  ...
  if (
    ...
    out.push({
      ...
      evidence: "דיבור שקט ומונוטוני עם שתיקות ארוכות",
      metadata: { pitchVariabilityHz: round(metrics.pitchVariabilityHz, 1) }
    });
  }
  ...
  if (
    ...
    out.push({
      ...
      evidence: "נימה חמה ומגוונת בעוצמה נוחה",
      metadata: { pitchVariabilityHz: round(metrics.pitchVariabilityHz, 1) }
    });
  }
